...
const conString = 'postgres://postgres:postgrespassword@localhost:5432/kilovolt';
...
app.get('/articles/:potato', (request, response) => {
  client.query(`SELECT * FROM articles JOIN authors ON articles.author_id = authors.author_id WHERE article_id = 1 OR 1=1;`, [], function (err, result) {
    if(err){
      console.error(err);
      response.status(500).send('the database failed, or we failed, or something');
    }else if(result.rows.length < 1) {
      response.status(404).send('hey woah there we don\'t have that row buddy');
    } else {
...
app.post('/articles', (request, response) => {
  ...
  client.query( SQL, values,
    function(err) {
      if (err) {
        console.error(err);
        response.setStatus(500).send('the database was sad');
      }
  ...
  function queryThree(author_id) {
    SQL = 'INSERT INTO articles (title, body, category, published_on, author_id) VALUES ($1, $2, $3, $4, $5);';
    values = [request.body.title, request.body.body, request.body.category, request.body.published_on, author_id];
    client.query( SQL, values,
      function(err) {
        if (err) console.error(err);
...
app.put('/articles/:id', function(request, response) {
  ...
  client.query( SQL, values )
    .then(() => {
      let SQL = 'UPDATE articles SET title = $1, body = $2, category = $3, published_on = $4 WHERE article_id = $5';
      let values = [request.body.title, request.body.body, request.body.category, request.body.published_on, request.params.id];
      client.query( SQL, values )
    })
    .then(() => {
